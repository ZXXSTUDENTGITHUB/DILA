...
for idx in range(len(X)):
    print(f"explain graph line {idx + 2}")

    if torch.isnan(labels[0].squeeze()):
        continue

    walks, masks, related_preds = explainer(
        X, edge_index, sparsity=sparsity, num_classes=num_class
    )

    x_collector.collect_data(masks, related_preds, labels[0].squeeze().long().item())
